...
function CreateTaskModal({ projectId, onClose, onDone }) {
  ...
  // );
const createMutation = useMutation({
  mutationFn: (payload) => projectAPI.createTask(projectId, {
    ...payload,
    status: 'active',
  }),
  onSuccess: () => {
...
const generateMutation = useMutation({
  mutationFn: (payload) => projectAPI.generateTasks(projectId, payload),
...
export default function AdminProjectDetails() {
 ...
 const { data, isLoading } = useQuery({
  ...
  const { data: generationStatus } = useQuery({
  queryKey: ['project-generation-status', id, queuedJobId],

  queryFn: () =>
    projectAPI
      .generationStatus(id, queuedJobId)
      .then((r) => r.data.data),
...
useEffect(() => {
  ...
  if (generationStatus.status === 'failed') {
    toast.error(generationStatus.failedReason || 'Queued generation failed');
  }
}, [generationStatus, completionToastShown, id, qc]);
