...
const { data: langData } = useQuery({
  queryKey: ['languages'],
  queryFn: () => projectAPI.languages().then((r) => r.data.data),
...
export default function AdminProjects() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => projectAPI.list({ page: 1, limit: 50, status: statusFilter || undefined, search: search || undefined }).then(r => r.data),
    placeholderData: (previousData) => previousData,
  });
 
  const statusMutation = useMutation({
    mutationFn: ({ id, status }) => projectAPI.setStatus(id, status),
    onSuccess: () => {
    ...
    <Layout>
      ...
      {showModal && (
        <CreateProjectModal
          ...
          onCreate={() => { setShowModal(false); qc.invalidateQueries({ queryKey: ['admin-projects'] }); }}
        />
      )}
      {showGenerateModal && (
        <GenerateProjectModal
          onClose={() => setShowGenerateModal(false)}
          onCreate={() => { setShowGenerateModal(false); qc.invalidateQueries({ queryKey: ['admin-projects'] }); }}
        />
      )}
    </Layout>
  );
