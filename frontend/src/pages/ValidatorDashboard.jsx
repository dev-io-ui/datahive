...
export default function ValidatorDashboard() {
  ...
  const { data: myValidations, isLoading: historyLoading } = useQuery(
    ['my-validations'],
    () => submissionAPI.myValidations({ limit: 5 }).then(r => r.data.data),
    { staleTime: 30000 }
  );

  const getSubmissionMutation = useMutation(
    () => submissionAPI.assignForReview(),
    {
      onSuccess: ({ data }) => {
  ...
  const submitValidationMutation = useMutation(
    (payload) => submissionAPI.validate(payload),
    {
      onSuccess: ({ data }) => {
        ...
        setRating(0);
        setFeedback('');
        qc.invalidateQueries(['my-validations']);
        refreshUser();
      },
    }
  );
