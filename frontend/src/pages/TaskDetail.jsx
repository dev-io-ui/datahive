...
export function TaskDetail() {
  ...
  const { data: taskData } = useQuery(
    ['task', id],
    () => taskAPI.get(id).then(r => r.data.data)
  );
...
export function SubmissionHistory() {
  ...
  const { data, isLoading } = useQuery(
    ['my-submissions', page, statusFilter],
    () => submissionAPI.mySubmissions({ page, limit: 20, status: statusFilter || undefined }).then(r => r.data),
...
export function WalletPage() {
  ...
  const { data: walletData } = useQuery(
    ['wallet'],
    () => walletAPI.summary().then(r => r.data.data)
  );

  const { data: txData, isLoading } = useQuery(
    ['transactions', page],
    () => walletAPI.transactions({ page, limit: 20 }).then(r => r.data),
    { keepPreviousData: true }
  );
...
export function AdminSubmissions() {
  ...
  const { data, isLoading } = useQuery(
    ['admin-submissions', page, statusFilter],
    () => submissionAPI.getAll({ page, limit: 20, status: statusFilter || undefined }).then(r => r.data),
    { keepPreviousData: true }
  );
