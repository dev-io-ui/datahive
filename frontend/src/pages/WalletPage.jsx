...
export default function WalletPage() {
  ...
  const { data: walletData, refetch } = useQuery('wallet', () => walletAPI.summary().then(r => r.data.data));
  const { data: txData, isLoading: txLoading } = useQuery(['transactions', txPage], () => walletAPI.transactions({ page: txPage, limit: 15 }).then(r => r.data), { keepPreviousData: true });
  const { data: wdData, isLoading: wdLoading } = useQuery('my-withdrawals', () => withdrawAPI.myList({ page: 1, limit: 20 }).then(r => r.data));

  const cancelMutation = useMutation((id) => withdrawAPI.cancel(id), {
    onSuccess: () => { toast.success('Cancelled. Amount refunded.'); qc.invalidateQueries('my-withdrawals'); refetch(); }
  });
    ...
    <Layout>
      ...
      {showWithdraw && (
        <WithdrawModal balance={wallet.balance || 0} onClose={() => setShowWithdraw(false)}
          onSuccess={() => { setShowWithdraw(false); qc.invalidateQueries('my-withdrawals'); qc.invalidateQueries('wallet'); setActiveTab('withdrawals'); }} />
      )}
    </Layout>
  );
}