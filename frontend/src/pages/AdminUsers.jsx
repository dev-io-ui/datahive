...
export default function AdminUsers() {
  ...
  const { data, isLoading } = useQuery(
    ...
    () => adminAPI.users({ page, limit: 20, search: search || undefined, role: roleFilter || undefined }).then(r => r.data),
    { keepPreviousData: true }
  );

  const statusMutation = useMutation(
    ({ id, status }) => adminAPI.updateStatus(id, status),
    { onSuccess: () => { toast.success('User status updated'); qc.invalidateQueries(['admin-users']); } }
  );

  const roleMutation = useMutation(
    ({ id, role }) => adminAPI.updateRole(id, role),
